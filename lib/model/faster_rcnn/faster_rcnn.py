...
#         ipdb.set_trace()
        ...
        base_feat = self.RCNN_base3(base_feat2) # backbone 最后输出特征
        ...
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes) # RPN 层
        ...
        if cfg.POOLING_MODE == 'align':
            pooled_feat = self.RCNN_roi_align(base_feat, rois.view(-1, 5)) # 没有FPN层结构 pooled_feat: torch.Size([256, 512, 7, 7])
        ...
        pooled_feat = self._head_to_tail(pooled_feat) # torch.Size([256, 4096])

        if self.context:
            feat1 = feat1.view(1, -1).repeat(pooled_feat.size(0), 1) # torch.Size([256, 128])
            feat2 = feat2.view(1, -1).repeat(pooled_feat.size(0), 1) # torch.Size([256, 128])
            feat3 = feat3.view(1, -1).repeat(pooled_feat.size(0), 1) # torch.Size([256, 128])
            pooled_feat = torch.cat((feat1, feat2, feat3, pooled_feat), 1) # torch.Size([256, 4480])
        ...
        cls_score = self.RCNN_cls_score(pooled_feat) # torch.Size([256, 9])
        cls_score_aux = self.RCNN_cls_score_aux(pooled_feat.detach()).view(pooled_feat.size(0), -1, self.num_aux1) # torch.Size([256, 9, 2])
